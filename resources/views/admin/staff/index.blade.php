    ...
    <div class="table-responsive">
      <table class="table align-items-center table-flush cashier-table" id="cashier-table">
        <thead class="thead-light">
          <tr><th>Cashier</th><th>Contact</th><th>Access</th><th>Orders</th><th>Sales</th><th>Last Sale</th><th>Actions</th></tr>
        </thead>
        <tbody>
          @forelse($rows as $cashier)
            ...
            <tr data-filter-row>
              ...
              <td><div class="cashier-contact"><strong>{{ $cashier->email }}</strong><span>{{ $cashier->number }}</span></div></td>
              <td>
              ...
              <td>
                <div class="action-group cashier-actions">
                  <a href="{{ route('admin.staff.edit', $cashier->id) }}" class="btn btn-sm btn-outline-primary" title="Edit cashier"><i class="bi bi-pencil-square"></i></a>
          ...
          @empty
            <tr data-filter-empty><td colspan="7" class="text-center py-5 text-muted">No cashiers yet. Add one to create the first cashier login.</td></tr>
          @endforelse
          @if($rows->count())
            <tr data-filter-empty style="display:none;"><td colspan="7" class="text-center py-5 text-muted">No matching cashiers found.</td></tr>
          @endif
        </tbody>
      </table>
    </div>
